    ...
    'authenticate': function (student_id, card_id, callback) {
        ...
        request.post({
            ...
            if (response.statusCode == 200){
                ...
                logger.info('error message: ' + data['reason']);
    ...
    'confirm': function (student_id, vote_token, callback) {
        request.post({
            ...
            if (response.statusCode == 200) {
                ...
                var station_id = '1';
                callback(null, station_id);
            } else {
                logger.error('error message: ' + data['reason']);
                callback(data['reason'], null);
            }

            // success
            callback(ballot + ' ' + callback_url);
